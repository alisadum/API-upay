...
class MerchantController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        $request->validate([
            ...
            'whatsapp' => 'sometimes|required|string|max:25|unique:merchants,whatsapp,' . Auth::user()->merchant->id,
    ...
    public function createPromotion(Request $request)
    {
        ...
        if (!empty($outletIds)) {
            $validOutlets = Outlet::whereIn('id', $outletIds)->where('merchant_id', $merchant->id)->where('is_approved', true)->count();
            if ($validOutlets !== count($outletIds)) {
                return response()->json(['message' => 'Salah satu outlet tidak valid atau belum disetujui.'], 403);
            }
            $location = null;
        }
        ...
        if (!empty($outletIds)) $promotion->outlets()->attach($outletIds);
    ...
    public function createOutlet(Request $request)
    {
        ...
        if ($request->hasFile('photo')) {
            $data['photo_path'] = $request->file('photo')->store('outlet_photos', 'public');
        }
        ...
        Notification::create([
            'user_id' => 1,
            'type' => 'new_outlet',
            'message' => "Outlet baru '{$outlet->name}' dari {$merchant->business_name} menunggu persetujuan.",
        ]);

        return response()->json([
            'message' => 'Outlet berhasil dibuat, menunggu persetujuan admin.',
            'data' => $outlet
        ], 201);
    }
    ...
    public function updateOutlet(Request $request, $id)
    {
        ...
        $outlet = Outlet::where('merchant_id', $merchant->id)->findOrFail($id);
        $data = [
            'name' => $request->name,
            'address' => $request->address,
            'city' => $request->city,
        ...
        if ($request->hasFile('photo')) {
            if ($outlet->photo_path) \Illuminate\Support\Facades\Storage::disk('public')->delete($outlet->photo_path);
            $data['photo_path'] = $request->file('photo')->store('outlet_photos', 'public');
        }
        ...
        Notification::create([
            'user_id' => 1,
            'type' => 'outlet_updated',
            'message' => "Outlet '{$outlet->name}' dari {$merchant->business_name} diperbarui, menunggu persetujuan.",
        ]);

        return response()->json([
            'message' => 'Outlet berhasil diperbarui, menunggu persetujuan admin.',
            'data' => $outlet
        ]);
    }
    ...
    public function deleteOutlet($id)
    {
        ...
        Notification::create([
            'user_id' => 1,
            'type' => 'outlet_deleted',
            'message' => "Outlet '{$outlet->name}' dari {$merchant->business_name} dihapus oleh merchant.",
        ]);
